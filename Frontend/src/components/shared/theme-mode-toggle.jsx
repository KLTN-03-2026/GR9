...
export default function ThemeModeToggle({ className = "" }) {
  ...
  return (
    ...
    >
      <span
        className={`theme-mode-toggle-thumb absolute left-1 top-1 h-8 w-8 rounded-full bg-primary shadow-lg shadow-primary/20 transition-transform duration-300 ease-out ${
          isDark ? "translate-x-[34px]" : "translate-x-0"
        }`}
      />
      <span className="relative z-10 grid h-8 w-full grid-cols-2 items-center">
        <span
          className={`theme-mode-toggle-icon flex h-8 w-8 items-center justify-center transition-all duration-300 ${
            isDark ? "text-on-surface-variant" : "text-primary-foreground"
          }`}
        >
          <Sun className={`h-4 w-4 transition-transform duration-300 ${isDark ? "-rotate-90 scale-75" : "rotate-0 scale-100"}`} />
        </span>
        <span
          className={`theme-mode-toggle-icon flex h-8 w-8 items-center justify-center transition-all duration-300 ${
        ...
        >
          <Moon className={`h-4 w-4 transition-transform duration-300 ${isDark ? "rotate-0 scale-100" : "rotate-90 scale-75"}`} />
        </span>
      </span>
    </Button>
  );
