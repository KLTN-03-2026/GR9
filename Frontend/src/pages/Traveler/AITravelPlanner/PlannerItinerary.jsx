...
function PlannerItinerary({ itinerary, handleGetLatLng }) {
  ...
  return (
    <div className="w-full min-w-0 rounded-[2rem] border border-outline-variant/15 bg-surface-container-lowest p-5 shadow-sm md:p-7 xl:flex xl:max-h-[calc(100vh-10rem)] xl:flex-col">
      <div className="mb-5 flex items-center justify-between gap-3 border-b border-outline-variant/10 pb-4">
        <div>
          ...
          <p className="mt-1 text-xs text-on-surface-variant md:text-sm">
            Chạm vào từng hoạt động để xem vị trí trên bản đồ.
          </p>
        </div>
        <div className="rounded-full bg-primary/8 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.18em] text-primary">
          {itinerary.itineraries.length} days
        </div>
      </div>

      <div className="space-y-8 xl:min-h-0 xl:flex-1 xl:overflow-y-auto xl:pr-2">
